...
describe('Sonar project boundaries', () => {
  ...
  it('assigns all ESI client analysis paths to the package project', () => {
    ...
    expect(csv(packageProperties, 'sonar.sources')).toStrictEqual(['src', 'scripts']);
    expect(csv(packageProperties, 'sonar.tests')).toStrictEqual(['tests']);
    expect(csv(packageProperties, 'sonar.javascript.lcov.reportPaths')).toStrictEqual([
      'coverage/lcov.info',
    ]);
  });

  it('limits root analysis to TypeScript projects in this checkout', () => {
    expect(csv(rootProperties, 'sonar.typescript.tsconfigPaths')).toStrictEqual([
      'tsconfig.json',
      'api/tsconfig.test.json',
      'features/member-audit/nuxt/tsconfig.json',
  ...
  it('retains generated, build, dependency, declaration, and coverage exclusions', () => {
    ...
    expect(csv(packageProperties, 'sonar.exclusions')).toStrictEqual(
...
function parseProperties(source: string): ReadonlyMap<string, string> {
  ...
  for (const line of source.replaceAll('\r\n', '\n').split('\n')) {
    ...
    if (separator < 1) {
      throw new Error(`Invalid Sonar property: ${line}`);
    }
    const key = normalized.slice(0, separator).trim();
    if (properties.has(key)) {
      throw new Error(`Duplicate Sonar property: ${key}`);
    }
...
function requiredProperty(properties: ReadonlyMap<string, string>, key: string): string {
  ...
  if (value === undefined) {
    throw new Error(`Missing Sonar property: ${key}`);
  }
  return value;
}
