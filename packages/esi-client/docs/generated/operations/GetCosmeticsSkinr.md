...
Cache response headers: `cache-control`, `etag`, `last-modified`. Cache extensions: `x-cache-age=31536000`, `x-cache-mode=ttl-based`, `x-client-cache-ttl=31536000`, `x-server-cache-mode=ttl-based`, `x-server-cache-ttl=604800`. Metadata-enabled and generic results expose normalized cache fields plus all original response headers.
...
Route rate limit: group `skinr`, maximum 12000 tokens per `15m` window.
