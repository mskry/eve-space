...
Cache response headers: `cache-control`, `etag`, `last-modified`. Cache extensions: `x-cache-age=2592000`, `x-client-cache-ttl=2592000`, `x-server-cache-mode=ttl-based`, `x-server-cache-ttl=2592000`. Metadata-enabled and generic results expose normalized cache fields plus all original response headers.
