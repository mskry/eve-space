...
Cache response headers: `cache-control`, `etag`, `last-modified`. Cache extensions: `x-cache-age=86400`, `x-client-cache-ttl=86400`, `x-server-cache-mode=ttl-based`, `x-server-cache-ttl=86400`. Metadata-enabled and generic results expose normalized cache fields plus all original response headers.
