...
Cache response headers: `cache-control`, `etag`, `last-modified`. Cache extensions: `x-cache-mode=not-cached`, `x-server-cache-mode=not-cached`. Metadata-enabled and generic results expose normalized cache fields plus all original response headers.
...
Route rate limit: group `routes`, maximum 3600 tokens per `15m` window.

Bounded request arrays: `body:$.avoid_systems` (maximum 1000), `body:$.connections` (maximum 1000). Maximum batch size: unspecified.
