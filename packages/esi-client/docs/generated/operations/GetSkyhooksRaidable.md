...
Cache response headers: `cache-control`, `etag`, `last-modified`. Cache extensions: `x-cache-age=300`, `x-cache-mode=ttl-based`, `x-client-cache-ttl=300`, `x-server-cache-mode=ttl-based`, `x-server-cache-ttl=60`. Metadata-enabled and generic results expose normalized cache fields plus all original response headers.
...
Route rate limit: group `activity`, maximum 30 tokens per `15m` window.
