...
import { generatedSchemaTestsComponent } from './schema-test-emitter.ts';
    ...
    async emit(context: EmitterContext) {
      ...
      for (const component of testComponents) {
        ...
        if (!Array.isArray(componentOutputs)) {
          throw new TypeError(
            `Generated test component ${component.name} did not return output paths`,
          );
        }
      ...
      return [{ target: testsTarget, kind: 'directory' as const }];
    },
  });
}
