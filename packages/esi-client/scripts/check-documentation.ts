...
export async function inspectDocumentationConsistency(
  ...
  if (!isRecord(model) || !Array.isArray(model.operations)) {
    throw new TypeError('Invalid normalized operation model for documentation validation');
  }
  if (
    !isRecord(provenance) ||
    ...
    throw new TypeError('Invalid OpenAPI provenance for documentation validation');
  }

  // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- only operations/operationId are used below, and are checked above
  const normalizedModel = model as unknown as NormalizedOpenApiModel;
  ...
  return {
    ...
    operationIds: normalizedModel.operations.map(({ operationId }) => operationId),
    provenance: {
...
function validateLinks(documents: ReadonlyMap<string, string>): void {
  for (const [sourcePath, content] of documents) {
    for (const match of content.matchAll(/!?\[[^[\]]*\]\(([^)]+)\)/gu)) {
      validateDocumentationLink(documents, sourcePath, match[1]?.trim());
    }
  }
}

function validateDocumentationLink(
  ...
  const resolvedTarget = resolveDocumentationTarget(sourcePath, decodedTarget);
  if (
    resolvedTarget === '..' ||
...
function decodeDocumentationTarget(
  sourcePath: string,
  rawTarget: string,
  pathTarget: string,
): string {
  try {
    return decodeURIComponent(pathTarget);
  } catch (error) {
...
function resolveDocumentationTarget(sourcePath: string, decodedTarget: string): string {
...
function assertCredentialSafe(path: string, content: string): void {
