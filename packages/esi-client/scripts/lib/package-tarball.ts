...
export async function extractPackageTarball(tarball: string, destination: string): Promise<string> {
  ...
  const state: ExtractionState = { globalPax: {}, nextLongPath: undefined, nextPax: {} };
  ...
  for (let offset = 0; offset + blockSize <= archive.length;) {
    ...
    if (applyExtendedHeader(entry, state)) {
      continue;
    }
...
function readArchiveEntry(archive: Buffer, offset: number): ArchiveEntry | undefined {
  ...
  if (bodyEnd > archive.length) {
    throw new Error('Truncated package tarball entry');
  }
  return {
...
function applyExtendedHeader(entry: ArchiveEntry, state: ExtractionState): boolean {
  if (entry.type === 'g' || entry.type === 'x') {
    ...
    if (entry.type === 'g') {
      state.globalPax = { ...state.globalPax, ...values };
    } else {
      state.nextPax = values;
    }
    return true;
  }
  if (entry.type === 'L') {
...
function parsePax(body: Buffer): Record<string, string> {
  ...
  while (offset < source.length) {
    ...
    if (separator < 0) {
      throw new Error('Invalid package tarball PAX record');
    }
...
function validateChecksum(header: Buffer): void {
  ...
  if (actual !== expected) {
    throw new Error('Invalid package tarball checksum');
  }
}
