...
export interface PlatformConfirmDialogOptions {
...
interface ConfirmDialogController {
  ...
  openConfirmDialog: (options: PlatformConfirmDialogOptions) => number
}

const confirmDialogKey: InjectionKey<ConfirmDialogController> = Symbol('platform-confirm-dialog')

export function providePlatformConfirmDialog() {
  const dialogOpen = ref(false)
  const dialogKey = ref(0)
  const actionPending = ref(false)
  const actionError = ref('')
  const current = shallowRef<PlatformConfirmDialogOptions>()
