...
Core migrations run in the explicit order declared by `api/src/db/migration-manifest.ts`; filesystem sorting is not an execution contract. Every file in `api/migrations` has one reviewed SHA-256 identity. To append a migration, choose a sequence greater than the manifest's current maximum, add the SQL file at the manifest tail, calculate its SHA-256 from the exact file bytes, and add that identity to the tail. Never insert before the tail, reuse a sequence, or edit a migration after it has been applied. `pnpm lint` rejects missing, extra, reordered, reused, or modified migration files.
