...
Core migrations run in the explicit order declared by `api/src/db/migration-manifest.ts`; filesystem sorting is not an execution contract. Every file in `api/migrations` has one reviewed SHA-256 identity, and `.gitattributes` forces those files to LF on every checkout. To append a migration, choose a sequence greater than the manifest's current maximum, add the SQL file at the manifest tail, calculate its SHA-256 from the exact LF file bytes, add that identity to the tail, and advance the accepted manifest length and fingerprint in the same change. The initial fingerprint remains unchanged so the documented duplicate sequence set cannot expand. Never insert before the tail, reuse a sequence, or edit a migration after it has been applied. `pnpm lint` rejects missing, extra, reordered, reused, unfrozen, or modified migration files.

The migration runner supports a fresh database and an exact interrupted prefix of the current manifest only. Core content identities are part of the runner-created ledger from its first row; missing identity storage or a missing row identity is unsupported. A startup identity error reports only its category and migration name and runs no pending SQL. For a disk-content mismatch, restore the reviewed migration file rather than changing its manifest identity. For an unsupported, missing, or changed database identity, preserve any evidence needed for diagnosis and recreate the disposable database with `docker compose down --volumes`; never fabricate a ledger row or checksum. There is no compatibility path for databases created from retired repository migration lines.
