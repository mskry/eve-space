...
This document describes EVE Space's technical architecture, security boundaries, persistence,
runtime operations, and recovery model.

## Overview

The application is a pnpm workspace with a Nuxt frontend, a Hono API, a separate Node worker,
PostgreSQL, and dedicated Redis roles for durable queues and disposable caching. Pinia Colada manages
typed in-memory frontend queries, Reka UI provides accessible interface behavior, and
`@evespace/esi-client` owns ESI access in the API and worker.

The browser calls the API directly for authenticated requests. Credentials, EVE tokens, encryption,
session handling, and character ownership checks stay in the API and worker and never move into
Nuxt. The sections below document the detailed architecture and security decisions. Related design
records and operational guides live in [this directory](./).
...
- `GET /api/organization/context` returns the caller's managed-organization, compliance, authority, and capability context.
...
`001_baseline.sql` creates the complete current core schema: users, one-to-many attached characters, encrypted per-character EVE tokens, intent-bound OAuth states, application and administrator sessions, deployment and module settings, transactional domain events, and SDE reference tables. Organization-owned tables include immutable organization epochs, managed corporations, corporation data sources and roster observations, account compliance and issues, character exceptions, role grants and authority evidence, custom groups, explicit member blocks, and a separately retained audit ledger. The migration runner maintains migration history, and a partial unique index prevents more than one main character per user. Migrations run automatically when the API container starts.

`001_baseline.sql` is the frozen root of core migration history. Core migrations run in the explicit manifest order declared by `api/src/db/migration-manifest.ts`; filesystem sorting is not an execution contract. Every file has one reviewed SHA-256 identity, and `.gitattributes` forces migration files to LF on every checkout. Each future migration must use a unique sequence greater than the current maximum and append its exact LF-byte identity at the manifest tail. Never insert, reorder, reuse a sequence, or edit an applied migration. `pnpm lint` rejects missing, extra, reordered, duplicate, non-tail, unfrozen, or modified migration files.

The migration runner supports a fresh database and an exact checksummed prefix of the current manifest only. Because the reset manifest contains only its root, prefix-resume tests append uniquely sequenced synthetic migrations to a test manifest; those test-only identities prove generic interrupted-prefix behavior without defining supported production history. Core content identities are part of the runner-created ledger from its first row; missing identity storage or a missing row identity is unsupported. A startup identity error reports only its category and migration name and runs no pending SQL. For a disk-content mismatch, restore the reviewed migration file rather than changing its manifest identity. A database created from a retired migration line, or carrying any unsupported, missing, or changed identity, has no compatibility path; preserve required diagnostic evidence and never fabricate a ledger row or checksum.

The approved destructive reset is permitted only for explicitly disposable database state. Stop the API and worker processes, then recreate PostgreSQL and durable Queue Redis from empty storage in the same maintenance window so no job from the retired database can execute against the baseline. Local Compose retains them in `postgres_data` and `queue_redis_data`; perform the approved reset with:

```bash
docker compose down --volumes
```

This command deletes both volumes. Do not use it for a database or queue that must be retained.
...
The 2026-09-06 private-envelope sizing sample warmed all 17 implemented read flows available to two fully scoped local-development characters. It produced a peak of 40 simultaneous private envelopes: serialized sizes were 400 B minimum, 539 B median, 195,369 B p95, and 196,287 B maximum; Redis `MEMORY USAGE` totaled 839,360 B. Asset pages and name batches accounted for the large upper tail. Scaling the measured aggregate to the four-character near-term traffic baseline, multiplying by 24 as a conservative hourly identity-churn allowance for the 24-hour retention window, and doubling for keys, allocator overhead, telemetry, and growth yields 80,578,560 B. Round that to a 128 MiB minimum and retain the 256 MiB `CACHE_REDIS_MAXMEMORY` operational target. Re-run the measurement after material attached-character or asset-page growth; do not infer payload contents from cache keys or export private envelope data.
...
Each allowed ESI upstream attempt runs inside an API-internal Effect scope. Transport admission acquires one distributed or conservative local permit, supervised renewal runs serially at half the permit TTL, and ownership remains held until both response-body consumption and SDK metadata/cooldown observation settle. Caller cancellation or permit loss reaches the active fetch/body, while a renewal already issued to Redis is awaited before exactly-once local release. Cache hits and pre-egress rejection create no scope. Core reads, core mutations, and installed-module platform dispatch keep their Promise contracts and existing retry, stale, timeout, mutation-revision, and post-dispatch cancellation policies; Effect is not exposed to Nuxt, feature modules, or public declarations. Runtime close stops admission and drains these finalizers before process-owned Redis connections are closed.
