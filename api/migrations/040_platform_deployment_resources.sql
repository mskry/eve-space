alter table platform_subject_lifecycles
  drop constraint platform_subject_lifecycles_organization_epoch_key,
  add constraint platform_subject_lifecycles_organization_epoch_key
    unique (subject_kind, organization_deployment_id, organization_version),
  drop constraint platform_subject_lifecycles_binding_check,
  add constraint platform_subject_lifecycles_binding_check check ((is_character_subject_kind(subject_kind) and character_id is not null and subject_id = character_id::text and organization_deployment_id is null and organization_version is null and corporation_source_id is null) or (subject_kind = 'alliance' and character_id is null and organization_deployment_id is not null and organization_version is not null and corporation_source_id is null) or (subject_kind = 'corporation' and character_id is null and organization_deployment_id is null and organization_version is null and corporation_source_id is not null) or (subject_kind = 'deployment' and character_id is null and subject_id = organization_deployment_id::text and organization_deployment_id is not null and organization_version is not null and corporation_source_id is null));

insert into platform_subject_lifecycles
  (subject_kind, subject_id, organization_deployment_id, organization_version, created_at)
select 'deployment', deployment_id::text, deployment_id, organization_version, created_at
from organization_epochs
on conflict (subject_kind, organization_deployment_id, organization_version) do nothing;
