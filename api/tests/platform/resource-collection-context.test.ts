...
test('loads the current organization context for a resource lifecycle', async () => {
  ...
  const context = { corporationId: 98_000_001, organizationVersion: 7 }
  mocks.sql.mockResolvedValue([context])

  await expect(
    loadResourceCollectionContext({ characterId: 9001, kind: 'character', lifecycleId }),
  ).resolves.toStrictEqual(context)
  expect(mocks.sql.mock.calls[0]?.slice(1)).toStrictEqual([lifecycleId])
})
