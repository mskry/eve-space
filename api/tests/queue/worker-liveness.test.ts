...
describe('scoped worker liveness', () => {
  test('accepts a fresh replica heartbeat without queue backlog input', () => {
    ...
    expect(evaluateScopedWorkerLiveness(heartbeatAt, now)).toStrictEqual({
      heartbeatAt,
      status: 'operational',
    })
  })
  ...
  test('does not expose missing, malformed, or stale heartbeat values', () => {
    const now = Date.parse('2026-09-10T12:00:00.000Z')
    expect(evaluateScopedWorkerLiveness(null, now)).toStrictEqual({
      heartbeatAt: null,
      status: 'stale',
    })
    expect(evaluateScopedWorkerLiveness('redis://private-host', now)).toStrictEqual({
      heartbeatAt: null,
      status: 'stale',
    })
    expect(
    ...
    ).toStrictEqual({ heartbeatAt: null, status: 'stale' })
    expect(evaluateScopedWorkerLiveness('2026-09-10', now)).toStrictEqual({
      heartbeatAt: null,
      status: 'stale',
    })
    expect(
  ...
  test('probes only the selected replica and closes its connection', async () => {
    ...
    await expect(probeScopedWorkerLiveness('worker-a')).resolves.toStrictEqual({
      heartbeatAt,
      status: 'operational',
    })
  ...
  test.each(['create', 'ping', 'get', 'close'] as const)(
    ...
    async (stage) => {
      ...
      if (stage === 'ping') {
        mocks.connection.ping.mockRejectedValueOnce(failure)
      }
      if (stage === 'get') {
        mocks.connection.get.mockRejectedValueOnce(failure)
      }
      if (stage === 'close') {
        mocks.close.mockRejectedValueOnce(failure)
      }
      ...
      expect(result).toStrictEqual(
        stage === 'close'
          ? { heartbeatAt, status: 'operational' }
          : { heartbeatAt: null, status: 'unavailable' },
      )
    },
  )
})
