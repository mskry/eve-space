...
describe('ESI egress verification', () => {
    ...
    const fixture = await createEgressFixture({
      'api/src/esi-gateway/internal/execution-runtime.ts': `
        ...
        class Layer {
          ...
          #executeSdkAttempt(attempt) { return attempt(this.#createTransport()) }
          execute() {
            return this.#executeSdkAttempt((transport) =>
              new EsiClient({
                fetch: transport,
  ...
  test('rejects an arbitrary transport variable in the execution owner', async () => {
    const fixture = await createEgressFixture({
      'api/src/esi-gateway/internal/execution-runtime.ts': `
        import { EsiClient } from '@evespace/esi-client'
        const transport = fetch
        export const client = new EsiClient({ fetch: transport })
      `,
    })
  ...
  test('discovers callable factories and rejects duplicate representation names', async () => {
    const fixture = await createEgressFixture({
      'api/src/characters/duplicate.ts': `
        import { createPublicEsiRead as read } from '../esi-gateway/feature-execution.js'
        export const first = read({ operation: 'status', name: 'duplicate-status' })
        export const second = read({ operation: 'status', name: 'duplicate-status' })
      `,
    })
  ...
  test('checks every installed feature server source against shared ESI policy', async () => {
    const fixture = await createEgressFixture({
      ...
      'features/alpha/server/src/raw-capabilities.ts': `
        ...
        export const resource = definePlatformSingleRequestResource({
          mode: 'single-request', operation: 'alpha-operation', request: () => ({
            accessToken: 'secret', principal: 'character-1',
            headers: { 'If-Modified-Since': 'yesterday' },
          }),
          map: ({ data }) => data, materialize: async () => createEsiExecutionRuntime,
        })
  ...
  test('accepts pure declared feature projections and type-only SDK imports', async () => {
    const fixture = await createEgressFixture({
      'features/alpha/server/src/resource.ts': `
        ...
        import { definePlatformSingleRequestResource } from '@eve-space/platform-module-contract/resources'
        export const resource = definePlatformSingleRequestResource({
          mode: 'single-request', operation: 'alpha-operation', request: ({ characterId }) => ({
...
function representationSource(name: string, extra = '') {
  return `
    ...
    export const value = createPublicEsiRead({
