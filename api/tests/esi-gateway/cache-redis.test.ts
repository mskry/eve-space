...
describe('cache Redis connections', () => {
  test('closing before first use does not create a connection', async () => {
    const { closeSharedCacheRedisConnection } = await import('../../src/cache-redis.js')
  ...
  test('keeps reconnecting with capped delays and rejects commands while unavailable', async () => {
    const { createCacheRedisConnection } = await import('../../src/cache-redis.js')
  ...
  test('counts connection errors by safe code without retaining messages', async () => {
    const [{ createCacheRedisConnection }, { getCacheConnectionErrorCounts }] = await Promise.all([
      ...
      import('../../src/esi-gateway/internal/telemetry-counters.js'),
    ])
    createCacheRedisConnection('redis://cache')
  ...
  test('clears the shared reference before waiting for close without closing it twice', async () => {
    ...
    const secondMock = mocks.instances[1]!

    expect(second).not.toBe(first)
    expect(mocks.instances).toHaveLength(2)
    expect(firstMock.quit).toHaveBeenCalledOnce()
    finishQuit!()
    await closing

    secondMock.status = 'ready'
    await closeSharedCacheRedisConnection()
    expect(secondMock.quit).toHaveBeenCalledOnce()
  })

  test('forgets a shared connection after graceful close falls back to disconnect', async () => {
    const { closeSharedCacheRedisConnection, getSharedCacheRedisConnection } =
      await import('../../src/cache-redis.js')
    const first = getSharedCacheRedisConnection()
    const firstMock = mocks.instances[0]!
    firstMock.status = 'ready'
    firstMock.quit.mockRejectedValue(new Error('Redis unavailable'))

    await closeSharedCacheRedisConnection()
    const second = getSharedCacheRedisConnection()
    const secondMock = mocks.instances[1]!

    expect(firstMock.disconnect).toHaveBeenCalledOnce()
    expect(second).not.toBe(first)
    secondMock.status = 'ready'
    await closeSharedCacheRedisConnection()
  })
