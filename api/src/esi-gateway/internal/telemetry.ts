...
type RefreshFailureClass = 'esi-cooldown' | 'esi-unavailable' | 'response-invalid' | 'unknown'

interface EsiDependencyStatus {
  readonly status: DependencyState
  readonly checkedAt: string
  readonly operationFailures?: number
}

interface EsiCacheDependencyStatus extends EsiDependencyStatus {
  readonly connectionErrors: Readonly<Record<string, number>>
  readonly envelopeRejections: {
...
interface EsiUpstreamOperationStatus {
  readonly operation: string
  readonly policy: {
    readonly authorization: 'public' | 'character'
    readonly cache: 'shared' | 'none'
    readonly freshness: string
    readonly rateGroup: string
    readonly declaredRateGroup: string | null
  }
  readonly observedRateGroup: string | null
  readonly rateGroupMismatches: number
  readonly cacheSources: Readonly<Record<CacheSource | 'stale', number>>
  readonly outcomes: Readonly<
    Record<
      'success' | 'notModified' | 'redirect' | 'rateLimited' | 'clientError' | 'serverError',
      number
    >
  >
...
export interface EsiStatusTelemetry {
  readonly checkedAt: string
  readonly cache: EsiCacheDependencyStatus
  readonly coordination: EsiDependencyStatus
  readonly cooldown: {
    readonly status: 'inactive' | 'active' | 'unavailable'
    readonly checkedAt: string
    readonly globalRetryAt: string | null
    readonly activeOperations: readonly { readonly operation: string; readonly retryAt: string }[]
  }
  readonly upstream: {
    readonly status: 'operational' | 'degraded' | 'unavailable'
    readonly checkedAt: string
    readonly operations: readonly EsiUpstreamOperationStatus[]
  }
}

export interface EsiUpstreamObservation {
  readonly status: 'operational' | 'degraded' | 'unavailable' | 'stale'
  readonly refreshFailureClass?: RefreshFailureClass
}
