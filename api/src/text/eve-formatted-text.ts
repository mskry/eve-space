...
export function parseEveFormattedText(
  ...
  if (!finalPlainText) return undefined
...
function parseTag(value: string, start: number): ParsedTag {
  ...
  if (end === -1) return { closing: false, end }

  const closing = value[start + 1] === '/'
  const nameStart = start + (closing ? 2 : 1)
  if (!isAsciiLetter(value.codePointAt(nameStart))) return { closing, end }

  let nameEnd = nameStart + 1
  while (isAsciiAlphaNumeric(value.codePointAt(nameEnd))) nameEnd += 1
...
function readFontTagColor(value: string, initialCursor: number, tagEnd: number) {
  ...
  while (cursor < tagEnd) {
    ...
    if (attribute.key === 'color') return parseEveColor(attribute.value)
...
function readTagAttribute(value: string, initialCursor: number, tagEnd: number) {
  ...
  if (keyStart === initialCursor) return undefined

  let separator = keyStart
  while (separator < tagEnd && value[separator] !== '=') separator += 1
  if (separator === tagEnd) return undefined
...
function readAttributeValue(value: string, start: number, tagEnd: number) {
  ...
  if (quote === "'" || quote === '"') {
    ...
    if (end === -1 || end > tagEnd) return undefined
    return { end: end + 1, value: value.slice(start + 1, end) }
  }

  let end = start
  while (end < tagEnd && !isAttributeBoundary(value, end)) end += 1
...
function parseEveColor(value: string | undefined) {
  ...
  if (!alphaRedGreenBlue) {
    ...
    if (withoutLongSuffix.startsWith('0x')) alphaRedGreenBlue = withoutLongSuffix.slice(2)
    else if (withoutLongSuffix.startsWith('#')) alphaRedGreenBlue = withoutLongSuffix.slice(1)
  }
  if (!alphaRedGreenBlue || !isEightHexDigits(alphaRedGreenBlue)) return undefined
...
function closeTagFrame(frames: TagFrame[], name: string) {
  for (let index = frames.length - 1; index >= 0; index -= 1) {
    if (frames[index]!.name !== name) continue
    frames.splice(index)
    return
  }
}
...
function normalizeTextRuns(runs: readonly TextRun[]) {
  ...
  for (const run of runs) {
    ...
    if (excessNewlines) text = text.slice(excessNewlines)
...
function findTagEnd(value: string, start: number) {
  ...
  if (attributeValue) return findAttributeTagEnd(value, ...attributeValue)
...
function fontAttributeValueStart(
  ...
  if (!startsWithIgnoreCase(value, start + 1, 'font')) return undefined
  if (startsWithIgnoreCase(value, start + 1, 'fontsize=')) return [start + 10, true]
  if (value[start + 5]?.toLowerCase() === 's') return undefined
...
function urlAttributeValueStart(
  ...
  if (!startsWithIgnoreCase(value, start + 1, 'url')) return undefined
...
function findAttributeTagEnd(value: string, initialCursor: number, initialValuePending: boolean) {
  ...
  while (cursor < value.length) {
    ...
    if ('tagEnd' in valueStart) return valueStart.tagEnd

    const valueEnd = scanAttributeValue(value, valueStart.cursor)
    if ('tagEnd' in valueEnd) return valueEnd.tagEnd
...
function scanAttributeName(value: string, initialCursor: number): AttributeScanResult {
  ...
  if (cursor === initialCursor) return { tagEnd: value.indexOf('>', cursor) }

  let separator = cursor
  while (separator < value.length) {
    if (value[separator] === '=') return { cursor: separator + 1 }
    if (value[separator] === '>') return { tagEnd: separator }
    separator += 1
  }
  return { tagEnd: -1 }
}
...
function scanAttributeValue(value: string, initialCursor: number): AttributeScanResult {
  ...
  if (quoteCharacters.has(quote)) {
    ...
    if (closingQuote === -1) return { tagEnd: value.indexOf('>', initialCursor) }
    return { cursor: closingQuote + 1 }
  }

  let cursor = initialCursor
  while (cursor < value.length && !isAttributeBoundary(value, cursor)) cursor += 1
  if (cursor === value.length) return { tagEnd: -1 }
  return { cursor }
}
...
function isLineBreakTag(value: string, start: number, end: number) {
  if (value.slice(start + 1, start + 3).toLowerCase() !== 'br') return false
...
function legacyUnicodeBody(value: string) {
  ...
  const quote = value[1]!
  if (!quoteCharacters.has(quote) || value.at(-1) !== quote) return undefined
