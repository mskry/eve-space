...
function callableOperationRegistrations(sources) {
  ...
  for (const { path, source } of sources) {
    ...
    visit(sourceFile, (node) => {
      if (!ts.isCallExpression(node) || !factories.has(calledFunctionName(node.expression))) {
        return
      }
...
function registeredRepresentationName(
  ...
  if (!registrationFunctions.has(calledFunctionName(node.expression))) {
    return
  }
  const definition = resolveInitializer(node.arguments[0], declarations)
  if (
...
function calledFunctionName(expression) {
  ...
  if (ts.isElementAccessExpression(value)) {
    return stringLiteralValue(value.argumentExpression)
  }
}
...
function callableFactoryFunctionsFor(sourceFile) {
  ...
  visit(sourceFile, (node) => {
    ...
    const specifier = stringLiteralValue(node.moduleSpecifier)
    const bindings = node.importClause?.namedBindings
    if (!specifier?.endsWith('/esi-gateway/feature-execution.js') || !bindings) {
      return
    }
    if (!ts.isNamedImports(bindings)) {
      return
    ...
    for (const element of bindings.elements) {
      const imported = element.propertyName?.text ?? element.name.text
      if (!element.isTypeOnly && canonical.has(imported)) {
        functions.add(element.name.text)
      }
    }
  })
...
function collectRepresentationDefinitionImport(node, canonical, functions) {
  ...
  const specifier = stringLiteralValue(node.moduleSpecifier)
  const bindings = node.importClause?.namedBindings
  if (!specifier?.endsWith('/esi-gateway/internal/representations.js') || !bindings) {
    return
  }
  if (!ts.isNamedImports(bindings)) {
    return
...
function registerRepresentationDefinitionAlias(name, initializer, functions) {
  ...
  if (!ts.isIdentifier(value) || !functions.has(value.text) || functions.has(name)) {
    return false
  }
  functions.add(name)
  return true
...
function staticPropertyName(name, declarations = new Map()) {
  ...
  if (ts.isComputedPropertyName(name)) {
    return staticStringValue(name.expression, declarations)
  }
}
...
function hasNamedProperty(path, source, names, ignoreCase = false) {
  ...
  visit(sourceFile, (node) => {
    ...
    if (name && names.has(ignoreCase ? name.toLowerCase() : name)) {
      found = true
    }
  })
  return found
...
function collectSdkClientReferences(sourceFile) {
  ...
  return { classes, declarations, factories, namespaces }
}
...
function collectNamedSdkClientImport(element, factories, classes) {
  ...
  const imported = element.propertyName?.text ?? element.name.text
  if (isSdkClientFactoryName(imported)) {
    factories.add(element.name.text)
  }
  if (imported === 'EsiClient') {
...
function resolveAliases(declarations, registerAlias) {
  ...
  while (changed) {
    ...
    for (const declaration of declarations) {
      changed = registerAlias(declaration) || changed
    }
  }
}

function registerSdkClientFactoryAlias(sourceFile, declaration, factories, namespaces) {
  if (registerIdentifierFactoryAlias(declaration, factories, namespaces)) {
    return true
  }
...
function registerSdkClientClassAlias(sourceFile, declaration, classes, namespaces) {
  if (registerIdentifierClassAlias(declaration, classes, namespaces)) {
    return true
  }
...
function collectSdkClientConstructionViolation(
  ...
  if (!isFactoryCall && !isGenericConstruction) {
    return
  }
  if (owner === 'core' && isGenericConstruction) {
    ...
    if (usesRequiredTransport(node, owner)) {
      return
    }
  }
  if (owner === 'core' && usesRequiredTransport(node, owner)) {
    return
  }
...
function usesRequiredTransport(call, owner) {
  ...
  if (!fetchProperty || !ts.isPropertyAssignment(fetchProperty)) {
    return false
  }
...
function isAttemptScopedTransport(construction, transport) {
  if (!ts.isIdentifier(transport) || transport.text !== 'transport') {
    return false
  }
...
function isSdkClientFactoryExpression(expression, factories, namespaces) {
  const value = unwrapExpression(expression)
  if (ts.isIdentifier(value)) {
    return factories.has(value.text)
  }
  if (ts.isPropertyAccessExpression(value)) {
    return (
      ts.isIdentifier(unwrapExpression(value.expression)) &&
  ...
  const nested = await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) {
        return loadSources(repositoryRoot, path, extensions)
      }
