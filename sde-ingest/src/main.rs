...
fn main() -> Result<()> {
    ...
    if let Some(current) = db::latest_build_projection(&mut client)? {
        if !db::needs_ingest(
            Some(current),
            latest.build_number,
        ...
        if current.build_number == latest.build_number {
            println!(
                "Local SDE build {} uses projection version {}; reloading projection version {INGEST_PROJECTION_VERSION}.",
                current.build_number, current.ingest_version
            );
        } else {
            println!(
                "Local build {} is behind the latest build {}.",
                current.build_number, latest.build_number
            );
        }
    } else {
...
fn ingest(
    ...
    let started = Instant::now();
    let (attribute_count, effect_count) =
        typed::ingest_type_dogma(&mut archive, &mut tx).context("ingesting type dogma")?;
    println!(
    ...
    println!(
        "Ingesting {} remaining datasets into sde_dataset_rows...",
        generic_members.len()
    );
