...
describe('buildHistoryTimeline', () => {
  it('sorts newest first and derives each completed period without mutating the source', () => {
    ...
    expect(buildHistoryTimeline(history)).toStrictEqual([
      { endDate: undefined, id: 3, startDate: '2024-01-01T00:00:00Z' },
      {
    ...
    expect(history.map((entry) => entry.id)).toStrictEqual([1, 3, 2])
  })
})
