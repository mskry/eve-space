create function eve_module_member_audit.persist_read_skill_evidence(input jsonb)
returns jsonb
language sql
stable
parallel unsafe
begin atomic
  select jsonb_build_object(
    ...
    (
      ...
      from trained_skill_snapshots as trained
    ...
    (
      select jsonb_build_object(
        'observationId'::text, queued.observation_id,
        'dtoRevision'::text, queued.dto_revision,
        'validatedAt'::text, queued.validated_at,
        'snapshot'::text, queued.snapshot
      )
      from skill_queue_snapshots as queued
      where queued.organization_version = (input ->> 'organizationVersion'::text)::bigint
        and queued.target_user_id = (input ->> 'targetUserId'::text)::uuid
        and queued.managed_member_lifecycle_id = (input ->> 'managedMemberLifecycleId'::text)::uuid
        and queued.character_id = (input ->> 'characterId'::text)::bigint
        and queued.character_lifecycle_id = (input ->> 'characterLifecycleId'::text)::uuid
        and queued.authorization_generation = (input ->> 'authorizationGeneration'::text)::integer
        and queued.disclosure_version = (input ->> 'disclosureVersion'::text)::integer
        and queued.section_activation_version = (input ->> 'sectionActivationVersion'::text)::integer
    )
  ) as result;
end;
...
begin atomic
  with trained as (
    ...
    select
      ...
      input -> 'snapshot'::text,
      (input ->> 'validatedAt'::text)::timestamptz as timestamptz
    where input ->> 'resourceId'::text = 'skill-queue'::text
    on conflict (character_id) do update
    set organization_version = excluded.organization_version,
        target_user_id = excluded.target_user_id,
        managed_member_lifecycle_id = excluded.managed_member_lifecycle_id,
        ...
        snapshot = excluded.snapshot,
        validated_at = excluded.validated_at
    where skill_queue_snapshots.validated_at < excluded.validated_at
      or skill_queue_snapshots.observation_id = excluded.observation_id
    returning 1 as "?column?"
  ), balance as (
