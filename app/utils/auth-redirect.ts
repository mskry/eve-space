...
export function getLocalAuthRedirect(value: unknown): string | undefined {
  ...
  if (!destination || isAuthorizationPath(destination.pathname)) {
    return
  }
...
function isAuthorizationPath(value: string) {
  ...
  for (let depth = 0; depth < 4; depth += 1) {
    ...
    if (!destination || ['/auth', '/auth/'].includes(destination.pathname)) {
      return true
    }
    if (!/%[\dA-Fa-f]{2}/.test(destination.pathname)) {
      return false
    }
    try {
